...
void test_pcap_ingestion() {
    ...
    // Also build adjacency using PathReconstructor (for path-based analysis)
    PathReconstructor pathReconstructor(*graph);
    pathReconstructor.build_adjacency();
    if (!nodes.empty()) {
        auto first_node_it = nodes.begin();
        cout << "PathReconstructor adjacency built for at least node ID: "
             << first_node_it->first << "\n\n";
    }
    ...
    if (!nodes.empty()) {
        ...
        cout << "=== First Node Properties ===\n";
        cout << "Node ID: " << first_node->getId() << "\n";
        cout << "IP: " << first_node->getIp() << "\n";
        cout << "Degree: " << first_node->get_degree() << "\n";
        cout << "In-degree: " << first_node->get_in_degree() << "\n";
        cout << "Out-degree: " << first_node->get_out_degree() << "\n";
        cout << "Unique ports: " << first_node->get_unique_ports() << "\n";
        cout << "Bytes sent: " << first_node->get_total_bytes_sent() << "\n";
        cout << "Bytes received: " << first_node->get_total_bytes_recv() << "\n";
        cout << "Is internal: " << (first_node->get_is_internal() ? "true" : "false") << "\n\n";
    }
    
    // Log first edge properties
    if (!edges.empty()) {
        auto first_edge = edges[0];
        cout << "=== First Edge Properties ===\n";
        cout << "Source node ID: " << first_edge->get_src() << "\n";
        cout << "Destination node ID: " << first_edge->get_dst() << "\n";
        cout << "Protocol: " << (int)first_edge->get_protocol() << " (6=TCP, 17=UDP)\n";
        cout << "Source port: " << first_edge->get_src_port() << "\n";
        cout << "Destination port: " << first_edge->get_dst_port() << "\n";
        cout << "Flags: " << (int)first_edge->get_flags() << "\n";
        cout << "Duration: " << first_edge->get_duration_ms() << " ms\n";
        cout << "Packets: " << first_edge->get_packetCount() << "\n";
        cout << "Bytes: " << first_edge->get_byteCount() << "\n";
        cout << "Avg packet size: " << fixed << setprecision(2) << first_edge->get_avg_packet_size() << " bytes\n";
        ...
        cout << "Time window: " << duration_sec.count() << "." << setfill('0') << setw(6) << duration_us << " seconds\n\n";
    }
